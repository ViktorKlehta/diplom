...
import pandas as pd
import numpy as np
from sklearn.base import BaseEstimator  # Добавьте, если модель использует sklearn
import numpy as np
import pandas as pd
...
def index(request):
    # sports=Sport.objects.order_by('id')
    # vid_sports = Sport.objects.values('vid_sporta').distinct()
...
def input(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if estate.latitude and estate.longitude:
                ...
                # Prepare the data for prediction in the same format as during training
                new_data = {
                    'Тип': [float(form.cleaned_data['type'])],
                    'Количество комнат': [float(form.cleaned_data['room_count'])],
                    'Тип дома': [form.cleaned_data['house_type']],  # Now string value
                    'Тип продажи': [form.cleaned_data['sale_type']],  # Now string value
                    'Улица': [form.cleaned_data['street']],  # New field
                    'Район': [form.cleaned_data['district']],  # New field
                ...
                # Make sure your model expects these exact column names and can handle string values
    ...
    # Format prices only if prediction was made
